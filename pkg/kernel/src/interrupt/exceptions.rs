...
pub unsafe fn register_idt(idt: &mut InterruptDescriptorTable) {
    ...
    idt.debug.set_handler_fn(debug_handle);
...
pub extern "x86-interrupt" fn debug_handle(stack_frame: InterruptStackFrame) {
...
pub extern "x86-interrupt" fn cp_protection_exception_handler(
    ...
    panic!(
        "EXCEPTION: CP PROTECTION EXCEPTION, ERROR_CODE: 0x{:016x}\n\n{:#?}",
        error_code, stack_frame
    );
}
